...
function LoginContent() {
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 flex items-center justify-center p-4">
            ...
            <div className="relative z-10 bg-slate-900/50 backdrop-blur-xl border border-slate-800 rounded-3xl shadow-2xl p-8 max-w-md w-full space-y-6">
                {/* Logo */}
                ...
                {tab === 'magic' && (
                    <div className="space-y-3">
                        <p className="text-slate-400 text-xs">Enter your email and we'll send a one-click login link. Works on mobile data.</p>
                        <input type="email" placeholder="Email address"
                            value={email} onChange={e => setEmail(e.target.value)}
                ...
                {tab === 'oauth' && (
                    <div className="space-y-3">
                        <div className="p-3 bg-amber-500/10 border border-amber-500/30 rounded-xl text-amber-300 text-xs">
                            ⚠️ OAuth requires a stable internet connection. If you're on mobile data in India, use Email or Magic Link instead.
                        </div>
