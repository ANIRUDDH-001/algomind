...
describe('Chat API (/api/chat)', () => {
    ...
    beforeEach(() => {
        ...
        vi.mocked(checkAndIncrementWeeklySession).mockResolvedValue({ allowed: true, sessionsUsed: 0, limit: 5, sessionsRemaining: 5, reason: 'within_limit' });
    ...
    it('16. Interview first turn allowed -> increments weekly interview usage', async () => {
        ...
        expect(res.status).toBe(200);
        expect(checkAndIncrementWeeklySession).toHaveBeenCalledWith('user-123', 'interview');
    });
    ...
    it('17. Non-interviewer prompt does not trigger weekly session checks', async () => {
        ...
        expect(res.status).toBe(200);
        expect(checkAndIncrementWeeklySession).not.toHaveBeenCalled();
    });
