              ...
              <TooltipProvider>
                <ErrorBoundary>
                  <TourProvider>
                    ...
                    {!hideNavbar && <Navbar />}
                    {/* Main Content Area */}
                    <main className={`flex-1 min-h-0 flex flex-col overflow-x-hidden overflow-y-auto scroll-container scrollbar-thin scrollbar-track-transparent scrollbar-thumb-indigo-500/20 md:pb-0 ${hideNavbar ? 'pt-0 pb-0' : 'pt-[var(--navbar-h,64px)] pb-[calc(var(--bottom-nav-h,56px)+env(safe-area-inset-bottom,0px))]'}`}>
                      {children}
                    </main>
                    <TourOverlay />
                    <Toaster />
