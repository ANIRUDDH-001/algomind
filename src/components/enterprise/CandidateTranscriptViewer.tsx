...
export function CandidateTranscriptViewer({ reportData, candidateName, onClose }: CandidateTranscriptViewerProps) {
    ...
    return (
        <div className="fixed inset-0 bg-[var(--surface-base)]/90 backdrop-blur-md z-[100] flex items-center justify-center p-4 sm:p-6 overflow-y-auto">
            <Card className="bg-[var(--surface-1)] border-white/10 w-full max-w-4xl shadow-2xl flex flex-col my-auto max-h-[90vh] overflow-hidden">
                <div className="p-4 sm:p-6 border-b border-white/8 flex justify-between items-center bg-[var(--surface-1)]/50 sticky top-0 z-10 shrink-0">
                    <div>
                    ...
                    <Button variant="ghost" onClick={onClose} size="sm" className="text-zinc-400 hover:text-white shrink-0 h-10 w-10 p-0">
                        <X className="w-5 h-5" />
                    </Button>
                </div>

                <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-8 custom-scrollbar bg-[var(--surface-base)]/50">
                    {questionsWithTranscript.length === 0 ? (
                        <div className="p-12 text-center text-zinc-500 bg-[var(--surface-1)]/50 rounded-xl border border-white/8">
                        ...
                        questionsWithTranscript.map((q: any, qIdx: number) => (
                            <div key={qIdx} className="space-y-6">
                                <div className="bg-[var(--surface-1)] border border-white/8 rounded-lg p-3 flex justify-between items-center sticky top-0 z-10 shadow-md">
                                    ...
                                    <div className="flex items-center gap-3 text-xs text-zinc-400 font-mono bg-[var(--surface-base)] px-2 py-1 rounded">
                                        ...
                                        <div>|</div>
                                        <div className="uppercase font-bold text-zinc-300">{q.status}</div>
                                    </div>
                                </div>
