...
export function CandidateHistoryTable({ submissions }: { submissions: Submission[] }) {
    ...
    const renderStatusBadge = (status: string) => {
        switch (status) {
            ...
            default: return <span className="px-2 py-1 rounded bg-[var(--surface-2)] text-zinc-400 font-bold border border-white/10 text-[10px] uppercase">{status}</span>;
        }
    };

    return (
        <div className="space-y-6">
            <Card className="bg-[var(--surface-1)] border-white/8 flex flex-col hover:border-blue-500/30 transition-all">
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left text-zinc-300 whitespace-nowrap">
                        <thead className="text-xs text-zinc-400 uppercase bg-[var(--surface-base)]/50 border-b border-white/8">
                            <tr>
                                ...
                                submissions.map((sub) => {
                                    ...
                                    return (
                                        <tr key={sub.id} className="border-b border-white/10 hover:bg-[var(--surface-2)]/30 transition-colors">
                                            ...
                                            <td className="px-6 py-4 text-center font-mono font-bold text-zinc-200 text-lg">
            ...
            {viewDetailsId && (
                <div className="fixed inset-0 z-[150] flex items-center justify-end bg-black/60 backdrop-blur-sm pointer-events-auto">
                    <div className="w-full md:w-[600px] h-full bg-[var(--surface-1)] border-l border-white/8 shadow-2xl flex flex-col transform transition-transform animate-in slide-in-from-right duration-300 relative">
                        {isLoadingReport || !reportData ? (
                            ...
                            <>
                                <div className="p-6 border-b border-white/8 flex justify-between items-start bg-[var(--surface-1)] sticky top-0 z-10">
                                    <div>
                                        <h3 className="text-xl font-bold text-white mb-1">Assessment Performance</h3>
                                        <p className="text-sm text-zinc-400">{reportData.campaign.title}</p>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <Button variant="ghost" onClick={() => setViewDetailsId(null)} className="text-zinc-400 hover:text-white">
                                            Close
                                        </Button>
                                    </div>
                                </div>
                                <div className="flex-1 overflow-y-auto p-6 space-y-8 report-print-area">
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="bg-[var(--surface-base)] p-4 rounded-xl border border-white/8 flex flex-col justify-center">
                                            <div className="text-xs text-zinc-500 uppercase tracking-wider mb-2 font-bold flex items-center gap-1.5"><TrendingUp className="w-3.5 h-3.5" /> Overall Score</div>
                                            <div className="text-3xl font-black text-white">
                                                {reportData.scores?.overall ? reportData.scores.overall.toFixed(1) : 'N/A'}
                                            </div>
                                        </div>
                                        <div className="bg-[var(--surface-base)] p-4 rounded-xl border border-white/8 flex flex-col justify-center">
                                            <div className="text-xs text-zinc-500 uppercase tracking-wider mb-2 font-bold flex items-center gap-1.5"><Brain className="w-3.5 h-3.5" /> Status</div>
                                        ...
                                        <div className="bg-[var(--surface-base)] p-4 rounded-xl border border-white/8">
                                            ...
                                            <div className="space-y-4">
                                                {reportData.questions.map((q: any, i: number) => (
                                                    <div key={i} className="flex flex-col gap-2 p-3 bg-[var(--surface-1)]/50 rounded-lg border border-white/10">
                                                        <div className="flex justify-between items-center">
                                                            <span className="font-bold text-zinc-200 text-sm truncate max-w-[200px]" title={q.title}>{i + 1}. {q.title}</span>
