...
export function RecommendationBanner() {
    ...
    return (
      <AnimatePresence>
        <motion.div
          ...
          className="relative rounded-2xl bg-gradient-to-br from-indigo-950/60 to-zinc-900/40 border border-indigo-500/20 p-5 overflow-hidden"
        >
          ...
          <div className="relative flex items-start gap-4">
            ...
            <button onClick={() => setDismissed(true)} className="text-zinc-600 hover:text-zinc-400 text-xs flex-shrink-0 mt-0.5">✕</button>
          </div>
        </motion.div>
      </AnimatePresence>
    );
  ...
  const nextConceptName = data.weakest?.[0]?.displayName ?? data.nextConcept;
  ...
  return (
    ...
    >
      <div className="w-9 h-9 rounded-xl bg-indigo-950/60 border border-indigo-500/20 flex items-center justify-center flex-shrink-0">
      ...
      <div className="flex gap-2 flex-shrink-0">
        <button
          onClick={() => router.push(`/learn/${data.nextConcept ?? data.weakest?.[0]?.slug}`)}
        ...
        <button
          onClick={() => router.push(`/interview?concept=${data.nextConcept}`)}
