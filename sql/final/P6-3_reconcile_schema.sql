...
ADD COLUMN IF NOT EXISTS leetcode_fetch_status TEXT DEFAULT 'idle',
ADD COLUMN IF NOT EXISTS leetcode_fetch_error TEXT;

-- Create leetcode_profiles table
...
-- RLS for leetcode_profiles
ALTER TABLE public.leetcode_profiles ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view own leetcode profile"
    ON public.leetcode_profiles FOR SELECT
    USING (auth.uid() = user_id);

CREATE POLICY "Service role can manage leetcode profiles"
    ON public.leetcode_profiles FOR ALL
    USING (true)
    WITH CHECK (true);

-- ============================================================
-- 3. ADMIN BOOTSTRAP (Safety)
-- ============================================================

-- This ensures the current admin_users table exists and has RLS enabled
