...
\restrict MIx8QFntD20lagAEzTtK47MkoqJW7UyPdYqeAMuREkqhtpnu1LHGUVoruEKpSWZ
...
BEGIN
  ...
  SELECT account_type INTO v_type FROM public.profiles WHERE id = auth.uid();

  RETURN QUERY SELECT
    (v_type = 'owner'),
    EXISTS(SELECT 1 FROM public.co_owners WHERE email = v_email),
...
CREATE POLICY "Admins and owners modify flags" ON public.global_feature_flags TO authenticated USING (((EXISTS ( SELECT 1
   FROM public.profiles
  WHERE ((profiles.id = auth.uid()) AND (profiles.account_type = ANY (ARRAY['admin'::text, 'owner'::text]))))) OR (EXISTS ( SELECT 1
   FROM public.co_owners
  WHERE (co_owners.email = (( SELECT users.email
           FROM auth.users
          WHERE (users.id = auth.uid())))::text))))) WITH CHECK (((EXISTS ( SELECT 1
   FROM public.profiles
  WHERE ((profiles.id = auth.uid()) AND (profiles.account_type = ANY (ARRAY['admin'::text, 'owner'::text]))))) OR (EXISTS ( SELECT 1
   FROM public.co_owners
  WHERE (co_owners.email = (( SELECT users.email
...
--
-- Name: co_owners Owner can manage co_owners; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Owner can manage co_owners" ON public.co_owners USING (public.is_owner());
...
\unrestrict MIx8QFntD20lagAEzTtK47MkoqJW7UyPdYqeAMuREkqhtpnu1LHGUVoruEKpSWZ
